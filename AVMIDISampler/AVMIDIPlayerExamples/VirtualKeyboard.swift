...
class VirtualKeyboard: UIView {
    ...
    required init?(coder aDecoder: NSCoder) { //Failable initializer
    ...
    // MARK: Update key status (relative sizing)
    private func updateKeyRects() {
        ...
        //then iterate through keys to determine black or white
        for i in 1 ..< keysNum {
            if !(isWhiteKey(i)) {
    ...
    private func updateKeyStates() {
        ...
        var currentKeyState = [Bool](repeatElement(false, count: keysNum))
        ...
        for i in 0 ..< count {
            let touch = touches[i] as! UITouch //Downcasting
        ...
        if keysUpdated {
            DispatchQueue.main.async { [unowned self] in
                self.setNeedsDisplay() //Update UI using draw method
            }
        }
    }
...
@objc protocol VirtualKeyboardDelegate {
